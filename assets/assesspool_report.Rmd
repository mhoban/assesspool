...
print(
  tagList(
    tags$h4('SNPs remaining after each filtering step.'),
    tags$h5(tags$small('Note: filtering results displayed are independent, not cumulative.'))
  )
)
filters <- filters %>%
  arrange(desc(count)) %>%
...
print(tags$h5(tags$small("Filter options with no effect: {unchanged}.")))
  ...
  group_walk(\(fst_data,method) {
    ...
    all_pools %>%
      iwalk(\(all_shared,hdr) {
        ...
        print(tags$h5("Global summary statistics by coverage cutoff"))

        # TODO: use error_y for SD of fst
        # plot_ly(
        #   x = ~cov,
        #   y = ~fst,
        #   type='scatter',
  ...
  group_walk(\(fst_data,method) {
    ...
    var_pairs %>%
      walk(\(trace_vars) {
        ...
        print(tags$h5(HTML(trace_vars$description)))

        # create cutoff slider
        cutoff_sel <- tags$div(
          class="heatmap-coverage-slider",
          tagList(
            tags$label(
              id=str_glue("cutoff-label-{method$method}-{trace_vars$name}"),
              `for`=str_glue("cutoff-sel-{method$method}-{trace_vars$name}"),
              str_glue('Minimum coverage: {params$min_cov}')
            ),
            tags$div(
              id=str_glue("cutoff-sel-{method$method}-{trace_vars$name}")
            )
          )
        )
        print(cutoff_sel)
        ...
        '
           $(function() {{
             $("#cutoff-sel-{method$method}-{trace_vars$name}").labeledslider({{
               min: {params$min_cov},
               max: {params$max_cov},
               step: {params$cov_step},
               tickInterval: {params$cov_step},
               change: function (e, ui) {{
                 ...
                 $("#cutoff-label-{method$method}-{trace_vars$name}").text(`Minimum coverage: ${{ui.value}}`);
...
methods %>%
  ...
  walk(\(pair) {
    ...
    print(tags$h5(tags$small(HTML("Note: Due to potentially large datasets, only a subsample of 1,000 F<sub>st</sub> values is shown here."))))
...
# TODO: do this with a slider
cat("# Fisher test plots {.tabset}\n\n")
print(tags$h5(tags$small("Static plots are presented due to the potential for very large datasets.")))
