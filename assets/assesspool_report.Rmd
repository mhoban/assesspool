...
cat("<h4>Bar plot showing SNPs remaining after each filtering step.</h4>\n\n")
cat("<h5><small>Note: filtering results displayed are independent, not cumulative.<br>Filters with no effect are not shown.</small></h5>\n\n")

filters <- params$filters %>%
  map(\(f) {
    tibble(name = f$value$filter, snps = f$value$count)
  }) %>%
  list_rbind() %>%
  arrange(desc(snps)) %>%
  mutate(
    name = forcats::fct_reorder(name,-snps),
    name = forcats::fct_relevel(name,"before"),
    filtered = snps[name == "before"] - snps,
    filtered = case_when(
      name == "before" ~ snps,
      .default = filtered
    )
...
t_fmt <- '<b>Before filtering: {num(snps[name == "before"])} SNPs<br>After filtering:  {num(snps)} SNPs</b>'
fig <- ggplot(filters) +
  geom_bar(aes(x=name,y=snps,fill=snps,text=str_glue(t_fmt)),stat="identity") +
...
fig <- fig %>%
  ...
  layout(
    xaxis = list(title = "Filter", fixedrange = TRUE),
...
```{r, fst-correlation, results='asis'}
