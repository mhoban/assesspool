...
if (show_filter) {
  filters <- read_tsv(params$filter,col_names = c('filter','count'))
} else {
  filters <- NULL
}
...
if (show_final_filter) {
  final_filters <- read_tsv(params$final_filter,col_names = c('filter','count'))
} else {
  final_filters <- NULL
}
  ...
  group_walk(\(fst_data,method) {
    ...
    pools_shared %>%
      iwalk(\(shared,hdr) {
        ...
        # summarize the data by cutoff level
        fst_grp <- fst_cov %>%
          group_by(cutoff) %>%
          summarise(
        ...
        fig <- fig %>%
          layout(
            ...
            title = list(text=str_glue("Summary statistics by coverage cutoff - {method$method}"), yref= 'paper', y=1, font=list(size=12)),
            showlegend = F,
          ...
          reduce(\(figs,cutoff) {
            ...
            if (is.null(figs$scatter_fig)) {
              ...
              figs$scatter_fig <- tagList(
                div(
                  `data-fig`=fig_id,
                  `data-json-prefix`=str_glue("scatter-json-{method$method}-{shr}"),
                  class="fig-container",
                  tagList(fig)
                )
              )
            ...
            json_tag <- tags$script(
              id = str_glue("scatter-json-{method$method}-{shr}-{cutoff}"),
            ...
            if (is.null(figs$box_fig)) {
              fig_id <- str_glue("box-fig-{method$method}-{shr}")
              fig$elementId <- fig_id
              figs$box_fig <- tagList(
                div(
                  `data-fig`=fig_id,
                  `data-json-prefix`=str_glue("box-json-{method$method}-{shr}"),
                  class="fig-container",
                  tagList(fig)
                )
              )
            ...
            fig_json <- fig %>%
              ...
              jsonlite::minify()
            json_tag <- tags$script(
              id = str_glue("box-json-{method$method}-{shr}-{cutoff}"),
              type = 'application/json',
              HTML(fig_json)
            )
            print(json_tag)
          ...
          walk(\(trace_vars) {
            ...
            heatmap_fig <- cutoffs %>%
              reduce(\(plot_fig,cutoff) {
                ...
                hmd <- full_table %>%
                  left_join(fst_grp,by=c("pop1","pop2")) %>%
                  ...
                  mutate(
                    # record whether value was NA
                    across(where(is.numeric),\(x) is.na(x),.names = "{.col}_na"),
                ...
                fig <- trace_vars$pairs %>%
                      ...
                      plot_fig %>%
                        {
                          if (sum(hmd[[str_glue("{v[3]}_na")]],na.rm = T) > 0 ) {
                            add_trace(
                              ...
                              hoverinfo = 'none',
                              data = hmd %>% 
                                filter( across( all_of(str_glue("{v[3]}_na")), \(na) na | is.na(na) ) ) %>%
                                mutate(  "{v[3]}" := if_else(.data[[str_glue("{v[3]}_na")]] == TRUE,0,NA) ),
                              x = ~.data[[v[1]]],
                              y = ~.data[[v[2]]],
                              z = ~.data[[v[3]]],
                              colorscale = 'Greys',
                        ...
                        add_trace(
                          ...
                          # zmin = ~min(.data[[ str_glue("{v[3]}_val") ]],na.rm = TRUE),#*0.00001,
                  ...
                  layout(
                    ...
                    # title = str_glue("{trace_vars$title} (coverage cutoff: &#x2265;{cutoff}")
                    title = list(
                ...
                if (is.null(plot_fig)) {
                  fig_id <- str_glue("heatmap-fig-{method$method}-{shr}-{trace_vars$name}")
                  fig$elementId <- fig_id
                  ...
                  plot_fig <- tagList(
                    div(
                      `data-fig`=fig_id,
                      `data-json-prefix`=str_glue("heatmap-json-{method$method}-{shr}-{trace_vars$name}"),
                      class="fig-container",
                      tagList(fig)
                    )
                  )
                ...
                json_tag <- tags$script(
                  id = str_glue("heatmap-json-{method$method}-{shr}-{trace_vars$name}-{cutoff}"),
                  type = 'application/json',
                  HTML(fig_json)
                )
                print(json_tag)
