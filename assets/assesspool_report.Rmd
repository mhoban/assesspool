...
print(
  tagList(
    h4('SNPs remaining after each filtering step.'),
    h5(tags$small('Note: filtering results displayed are independent, not cumulative.'))
  )
)
filters <- filters %>%
  arrange(desc(count)) %>%
...
print(h5(tags$small("Filter options with no effect: {unchanged}.")))
