...
<script>
// update any visible figures that are set to a different coverage cutoff
...
$(function() {
  ...
  $("#coverage-slider").labeledslider({
    min: `r params$nf$report_min_coverage`,
    max: `r params$nf$report_max_coverage`,
    step: `r params$nf$report_coverage_step`,
    tickInterval: `r params$nf$report_coverage_step`,
    slide: function (e,ui) {
      $('#coverage-label').text(`Minimum coverage: ${ui.value}`)
...
<div id="TOC2" class="sliderbar tocify" style="display: none">
  <label id="coverage-label" for="coverage-slider">Minimum coverage: `r params$nf$report_min_coverage`</label>
              ...
              reduce(\(plot_fig,cutoff) {
                ...
                hmd <- full_table %>%
                  ...
                  mutate(
                    ...
                    # this is so we can show grey squares for missing data
                    ...
                    if (!all(is.na(v))) {
                      ...
                      plot_fig %>%
                        add_trace(
                          ...
                          colorscale = cscale(v[4],n = 10,na = v[6]), # color palette, based on 100 gradations
                          # style the colorbar
                          colorbar = list(
